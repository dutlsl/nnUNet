...
def generate_pseudo_labels(
    ...
    if not os.path.exists(checkpoint_path):
        checkpoint_path = join(fold_dir, 'checkpoint_final.pth')
        ...
        for batch_start in range(0, len(png_files), batch_size):
            ...
            with torch.no_grad():
                ...
                if isinstance(logits, (list, tuple)):
                    ...
                    logits = logits.get('seg_logits', list(logits.values())[0])
...
if __name__ == '__main__':
    ...
    model_dir = os.path.join(project_root, 'nnUNet_results', 'Dataset600_OpenEDS2019', 'nnUNetTrainer_Vivim__nnUNetPlans__2d')
    if not os.path.exists(model_dir):
        model_dir = os.path.join(project_root, 'nnUNet_results', 'Dataset600_OpenEDS2019', 'nnUNetTrainer_ImageNetPretrained__nnUNetPlans__2d')

    seq_root = os.path.join(project_root, 'Openedsdata2019', 'Sequence_Dataset')
    if not os.path.exists(seq_root):
        seq_root = os.path.join(project_root, 'Openedsdata2019', 'Sequence_Extracted')

    output_root = os.path.join(project_root, 'Openedsdata2019', 'Sequence_PseudoLabels_400')
